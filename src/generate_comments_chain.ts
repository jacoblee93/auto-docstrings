...
import {
  COMMENT_GENERATION_HUMAN_TEMPLATE,
  COMMENT_GENERATION_SYSTEM_TEMPLATE,
  RESEARCH_AGENT_PREFIX,
} from "./prompts.js";
...
export async function createGenerateCommentsChain(): Promise<RunnableSequence> {
  ...
  // processing prior files. Move into the function to save tokens and reduce potential distractions
  // to the LLM.
  const researchAgentExecutor = await initializeAgentExecutorWithOptions(
    [tool],
    new ChatOpenAI({
    ...
    ts_doc_comments: z.array(
      z.object({
        ...
        params: z
          ...
          .describe(
            "For methods, a list of parameters the method takes. Should contain all information to render as an @param declaration. Should be empty for non-methods",
          ),
        returns: z
          .optional(z.string())
          .describe("For methods, information on what the method returns"),
      }),
    ),
  });
    ...
    functions: [
      {
        ...
        description:
          "Inserts TSDoc comments into code based on input arguments",
