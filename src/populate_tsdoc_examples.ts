...
const filePaths = (await walk(directoryPath))
  ...
  .filter((filePath) => {
    ...
    const relevantNode = findDeclarationNodeWithMissingExample(sourceNode);
    if (!relevantNode) {
...
function findDeclarationNodeWithMissingExample(
  rootNode: ts.SourceFile,
  nodeName?: string,
  nodeType?: "method" | "class" | "function",
...
console.log(await vectorstore.similaritySearch(filePaths[0]));
...
//     const declarationNode = findDeclarationNodeWithMissingExample(
//       sourceNode,
//       comment.name,
//       comment.type,
//     );
